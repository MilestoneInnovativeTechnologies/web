...
@section("content")
@php
$Data = \App\Models\PrivateArticle::find(Request()->code)->load('Audience');
$str_TarOpts = DB::select(DB::raw('SHOW COLUMNS FROM private_articles WHERE Field = "target"'))[0]->Type;
$str_TarTypOpts = DB::select(DB::raw('SHOW COLUMNS FROM private_articles WHERE Field = "target_type"'))[0]->Type;
$Options = [];
...
<div class="content"><form method="post" enctype="multipart/form-data">{{ csrf_field() }}
	<div class="panel panel-default">
		<div class="panel-heading"><strong>{{ $Data->title }}</strong><div class="pull-right">{!! PanelHeadBackButton((url()->current() == url()->previous())?Route('pra.index'):url()->previous()) !!}</div></div>
		<div class="panel-body pull-left" style="width: 450px;">
			...
			<hr>
			<div class="panel panel-default filter_partner filter_panel">
				<div class="panel-heading"><span class="panel-title" style="font-weight: 100">Filter Partners</span><a href="javascript:TogglePanelView('filter_customer')" title="" class="btn btn-default btn-xs pull-right toggle_anchor"><span class="glyphicon glyphicon-plus"></span> </a>
				</div>
				<div class="panel-body">
				@component('partner.comp_form_filter') @endcomponent
				</div>
				<div class="panel-footer clearfix"><a href="javascript:ReloadPartners()" class="btn btn-sm btn-default pull-right">Reload Partners</a></div>
			</div>
