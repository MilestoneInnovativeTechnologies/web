...
@section("content")
@php $ORM = \App\Models\Customer::with(['Cookies'])->latest(); @endphp
...
<div class="content">
	<div class="panel panel-default">
		<div class="panel-heading"><strong>Customer Cookies</strong></div>
		<div class="panel-body">
			...
			<div class="table-responsive">
				<table class="table table-bordered">
					<thead><tr><th>Name</th><th>Cookies</th><th>Action</th></tr></thead>
					<tbody>@foreach($Data as $Obj)
						<tr>
							<td>{{ $Obj->name }}</td><td>{!! GetCookies($Obj) !!}</td><td>{!! glyLink(Route('tscc.create',['code'=>$Obj->code]),'Add/Edit Cookie','transfer') !!}</td>
						</tr>
					@endforeach</tbody>
				</table>
			</div>@else
...
function GetCookies($Obj){
	...
	return $Cookies->map(function($item){ return '<dl><dt>'.$item->name.'</dt><dd>'.$item->value.'</dd></dl>'; })->implode('');
}
@endphp