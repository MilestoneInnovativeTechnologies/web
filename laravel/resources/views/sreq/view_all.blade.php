...
@section("content")
@php $SReq = new \App\Models\ServiceRequest; $ORM = $SReq->withoutGlobalScope('active')->where(function($Q){ $Q->where('status','INACTIVE'); }); @endphp
...
<div class="content">
	<div class="panel panel-default">
		<div class="panel-heading"><strong>Service Requests</strong>{!! PanelHeadBackButton((url()->current() == url()->previous())?Route('sreq.index'):url()->previous()) !!} <div class="pull-right">&nbsp;</div> @foreach(array_diff_key($SReq->actions,array_fill_keys($SReq->list_actions,'-')) as $action) {!! PanelHeadAddButton(Route('sreq.'.$action),$SReq->action_title[$action]) !!} @if($loop->remaining) <div class="pull-right">&nbsp;</div> @endif @endforeach</div>
