<div class="table-responsive"><table class="table table-bordered"><thead><tr><th>Seq</th><th>Details</th><th>Created</th><th>Assigned</th><th>Status</th></tr></thead><tbody>
	@forelse($Tasks as $t)
	<tr><th>{{ $t->seqno }}</th><td><a href="{{ Route('task.panel',$t->id) }}" style="color: inherit">{{ $t->title }}</a> @if($t->support_type) <em><small>({{ $t->Stype->name }})</small></em> @endif @if($t->description)<br><small>{!! nl2br($t->description) !!}</small>@endif</td><td>By: {{ $t->CreatedBy->name }}<br>On: <small>{{ date('d/M/y h:i a',strtotime($t->created_at)) }}</small></td><td>@if($t->Responder)To: <strong><a href="{{ Route('supportagent.panel',$t->Responder->responder) }}" style="color: inherit">{{ $t->Responder->Responder->name }}</a></strong><br>By: {{ $t->Responder->Assigner->name }}<br>On: {{ date('d/M/y h:i a',strtotime($t->Responder->created_at)) }}@endif</td><td>{{ $t->Cstatus->status }}@if($t->Cstatus->status_text)<br><small>{!! nl2br($t->Cstatus->status_text) !!}</small>@endif<br><small>({{ SecondDiff(time()-($t->Cstatus->start_time)) }})</small></td></tr>
	@empty
