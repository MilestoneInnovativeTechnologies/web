...
<div class="content">
	<div class="panel panel-default">
		<div class="panel-heading"><strong>Hold Task</strong>{!! PanelHeadBackButton((url()->previous() == url()->current())?(Route('tsk.index')):(url()->previous())) !!}</div>
		<div class="panel-body clearfix">
			<div class="row clearfix" style="margin-bottom: 10px;">
				<div class="col-md-6"><form method="post">{{ csrf_field() }}
					{!! formGroup(1,'status_text','textarea','Reason for Holding task',['style'	=>	'height:80px']) !!}
					<label class="checkbox-inline"><input type="checkbox" name="hold_ticket" value="yes"> <strong>Hold Ticket Also</strong></label>
					<div style="display: none" class="hold_ticket_status_text">
						<div class="form-group clearfix form-horizontal">
							<label class="control-label col-xs-4" style="text-align: left;">Ticket Status Text</label>
							<div class="col-xs-8">
								@foreach($Task->hold_predefined_customer_text as $SText)
								<div class="radio"><label><input type="radio" name="ticket_status" value="{{ $SText }}"> {{ $SText }}</label></div>
								@endforeach
								<div class="radio"><label><input type="radio" name="ticket_status" value="custom_status_text"> Custom Status</label></div>
							</div>
						</div>
						{!! formGroup(1,'ticket_status_text','textarea','Custom Status for ticket <small>(Customer side visible)</small>',['style'	=>	'height:120px;']) !!}
					</div>
					<div class="clearfix">
						<input type="submit" name="submit" value="Submit Reason" class="btn btn-primary pull-right">
					</div>
				</form></div>
				<div class="col-md-6">
...
$(function(){
	$('[name="ticket_status_text"]').parents('.form-group').slideUp();
	$('[name="ticket_status"]').click(function(){
		if(this.value == 'custom_status_text') $('[name="ticket_status_text"]').parents('.form-group').slideDown();
		else $('[name="ticket_status_text"]').parents('.form-group').slideUp();
	});
	$('[name="hold_ticket"]').click(function(){
		if(this.checked) $('.hold_ticket_status_text').slideDown();
		else $('.hold_ticket_status_text').slideUp();
	});
