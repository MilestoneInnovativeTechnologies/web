<div class="table-responsive"><table class="table table-bordered"><thead><tr><th>No</th><th>Status</th><th>User &amp; Time</th><th>Total Time</th></tr></thead><tbody>
	@forelse($Status as $s)
	<tr><th>{{ $loop->iteration }}</th><td>{{ $s->status }}@if(in_array($s->status,['HOLD'])) <br><small>{!! nl2br($s->status_text) !!}</small> @endif</td><td nowrap style="font-size: 12px;">{{ $s->User->name }}<br>{{ date('d/m/y h:i a',$s->start_time) }}</td><td nowrap style="font-size: 13px;">@if($s->total) {{ SecDiff($s->total) }} @endif</td></tr>
	@empty
