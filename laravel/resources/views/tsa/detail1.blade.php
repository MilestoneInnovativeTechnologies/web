...
@section("content")
@php
$Agent = \App\Models\TechnicalSupportAgent::whereCode(Request()->code)->with(['Details','Distributor','Tasks' => function($Q){ $Q->with('CreatedBy','Responder.Assigner','Ticket.Customer'); }])->first();
$StatusReport = GetStatusReport($Agent->Tasks);
//dd($Agent->Departments->implode('Department.name',', '))
@endphp

<div class="content">
	<div class="row">
		<div class="col col-md-9">
			<div class="panel panel-default"><div class="panel-heading"><div class="panel-title">{{ $Agent->name }}</div></div><div class="panel-body">
				<div class="table-responsive"><table class="table striped"><tbody>
					<tr><th>Agent Code</th><th>:</th><td>{{ $Agent->code }}</td><th>Phone</th><th>:</th><td>{!! PartnerPhone($Agent->Details) !!}</td></tr>
					<tr><th>Agent Name</th><th>:</th><td>{{ $Agent->name }}</td><th>Email</th><th>:</th><td>{!! PartnerEmails($Agent->Logins) !!}</td></tr>
					<tr><th>Support Team</th><th>:</th><th><a href="{{ Route('supportteam.panel',$Agent->Distributor->ParentDetails->code) }}" style="color: inherit">{{ $Agent->Distributor->ParentDetails->name }}</a></th><th>Departments</th><th>:</th><td>{{ $Agent->Departments->implode('Department.name',', ') }}</td></tr>
				</tbody></table></div>
			</div></div>
		</div>
	</div>
	...
	<div class="panel panel-default"><div class="panel-heading"><div class="panel-title">Active Tasks</div></div><div class="panel-body">
	@component('tsk.comp_tasks',['Tasks' => $Agent->Tasks->filter(function($Item){ return $Item->Cstatus->status != 'CLOSED'; })]) @endcomponent
	</div></div>
</div>
