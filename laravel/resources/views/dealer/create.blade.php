...
<div class="content">
	<form class="form dealer_{{ ($Update)?'update':'create' }}" method="post" action="{{ ($Update) ? Route('dealer.update',["code"	=>	$Code]) : Route('dealer.store') }}">
		{!! ($Update) ? (method_field('PUT') . csrf_field()) : csrf_field() !!}
		<div class="panel panel-default">
			<div class="panel-heading"><strong>{{ ($Update)?'Edit':'New'}} Dealer</strong><a href="{{ Route('dealer.index') }}" class="btn btn-default pull-right btn-sm"><span class="glyphicon glyphicon-arrow-left"></span> Back</a></div>
			...
			<div class="panel-footer clearfix">
				<div class="pull-right">
					<input type="submit" name="submit" value="{{ ($Update)?'Update':'Create'}} Dealer" class="btn btn-info">
				</div>
			</div>
		</div>
	</form>
