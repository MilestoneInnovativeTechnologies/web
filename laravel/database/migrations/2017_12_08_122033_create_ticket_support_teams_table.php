...
class CreateTicketSupportTeamsTable extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        Schema::create('ticket_support_teams', function (Blueprint $table) {
					$table->increments('id');
					$table->char('ticket',15)->index();
					$table->char('customer',15)->index()->nullable();
					$table->char('team',15)->index()->nullable();
					...
					$table->foreign('team')->references('code')->on('partners')->onUpdate('cascade')->onDelete('set null');
    ...
    {
        Schema::dropIfExists('ticket_support_teams');
    }
}
